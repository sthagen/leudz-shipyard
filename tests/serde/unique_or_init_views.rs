use shipyard::views::{UniqueOrInitView, UniqueOrInitViewMut};
...
#[test]
fn unique_or_init_view_serialize() {
    let world = World::new();
    ...
    world.run(|unique: UniqueOrInitView<GameState>| {
...
#[test]
fn unique_or_init_view_mut_serialize() {
    let world = World::new();
    ...
    world.run(|unique: UniqueOrInitViewMut<GameState>| {
        let serialized = serde_json::to_string(&unique).unwrap();
...
#[test]
fn unique_or_init_view_mut_deserialize_in_place() {
    let world = World::new();
    ...
    world.run(|mut unique: UniqueOrInitViewMut<GameState>| {
    ...
    world.run(|unique: UniqueOrInitView<GameState>| {
        assert_eq!(**unique.get().unwrap(), updated_state);
    });
}
...
fn unique_or_init_view_mut_direct_deserialize_panic() {
    serde_json::from_str::<UniqueOrInitViewMut<GameState>>("").unwrap();
}
